...
applyIf :: Env -> [MVal] -> (MVal, Env)
applyIf env [b, c, d] = case fst $ eval env b of
  MNil -> eval env d
  MFalse -> eval env d
  _     -> eval env c
applyIf env [b, c] = applyIf env [b, c, MNil]
applyIf env mv = (MList mv, env)

-- applyFn :: Env -> [MVal] -> (MVal, Env)
-- applyFn env [(MList [])]
...
eval env (MList (MSym "if" : rest)) = applyIf env rest
eval env (MList (MSym "do" : rest)) = (last $ map (evalAST env) rest, env)
-- eval env (MList (MList [MSym "fn*", MList binds, ast] : exprs)) = (fst $ eval env' ast, env)
--   where
--     env' = doBinds binds exprs $ outerC env
eval env (MList [MSym "fn*", MList [], ast]) = (MList [fst $ eval env ast], env)
eval env (MList [MSym "fn*", MList binds, ast]) = (MFun fn, env)
  where
    fn :: [MVal] -> MVal
    fn args = fst $ eval (doBinds binds args $ outerC env) ast
